...
exports.HOST = 'https://www.getonbrd.com';
exports.HOST_CL = 'https://www.getonbrd.cl';
const SEARCH_URL = `${exports.HOST}/webpros/search_jobs`;
...
const GetOnBrd = (session, defaultOpts = {}) => __awaiter(this, void 0, void 0, function* () {
    ...
    let csrfToken = '';
    if (session) {
        const $ = yield helpers_1.dom(exports.HOST, { headers: { cookie: sessionCookie } });
    ...
    const getJob = (url, gotOpts = defaultOpts) => __awaiter(this, void 0, void 0, function* () {
        ...
        const isClosed = helpers_1.txt($('h3.size2')).startsWith('Empleo finalizado');
        const common = {
            date,
        ...
        if (isClosed)
            return common;
        return Object.assign({}, common, { salary, category: {
                name: helpers_1.txt(_category),
                slug: _category.attr('href').match(/.+\/(.+)/)[1],
            }, tags: $('[itemprop="skills"] a')
                .map((i, el) => $(el).text())
                .get(), level: helpers_1.txt($('[itemprop="qualifications"]')), type: helpers_1.txt($('[itemprop="employmentType"]')), trending: $('.fa-fire').length > 0, country: helpers_1.txt(_loc.find('[itemprop="addressCountry"]')), city: helpers_1.txt(_loc.find('[itemprop="addressLocality"]')) });
    });
    ...
    const getCategories = (gotOpts = defaultOpts) => __awaiter(this, void 0, void 0, function* () {
        const $ = yield helpers_1.dom(exports.HOST_CL, gotOpts);
        return $('.bg-white a[href^="/emp"]')
            .map((i, el) => exports.HOST + $(el).attr('href'))
            .get();
    });
